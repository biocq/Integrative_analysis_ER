---
title: "Part 3: EXPRESSION ANALYSIS WITH AMNIOTE AND PRIMATE SPECIES"
...
```{r}
input1 <- "NormalizedRPKM_ConstitutiveExons_Amniote1to1Orthologues.txt" 
...
data<- read.delim(paste0("/home/jie/projects/ER_proj/part3/",input1)) #Amniote RPKM data
data[2:9]<-NULL  #Columns that contain other species ENSEMBL ID
ensembl <- read.table(paste0("/home/jie/projects/ER_proj/part3/",input2), sep="\t",header=TRUE)#import the ensembl file that contains ENSEMBL ID and matching GeneNames
...
replaced <- joined2 %>% dplyr::select(Class,Symbol, everything()) #place the last column to first column
