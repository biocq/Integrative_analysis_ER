---
title: "Cancer and Normal RNA Expression analysis using the stage information"
...
input1 <- "TcgaTargetGtex_rsem_gene_tpm_selected_genes.tsv"
data<-read.delim(paste0(Prefix,"part5/",input1))
```
...
ensembl <- read.table(paste0(Prefix,"part5/",input2), sep="\t",header=TRUE)#import the ensembl file that contains ENSEMBL ID and matching GeneNames
...
replaced <- joined %>% dplyr::select(Symbol, everything()) #place the last column to first column
replaced <- replaced %>% dplyr::select(Class, everything()) #place the last column to first column
replaced$Class<-NULL
...
```{r}
input3 <- "clinical.tsv"
clinical<- read.delim(paste0(Prefix,"part5/",input3))#clinical.tsv
...
withclinical <- withclinical %>% dplyr::select(sample,sample2,stage, everything())
...
write.table(withclinical3,file="ER.TcgaTargetGtex_tpm_with_clinical.tsv",row.names=FALSE, quote=FALSE, sep="\t",col.names=TRUE) #export
```


```{r}

input4 <- "TcgaTargetGTEX_phenotype.txt"
phenotype<- read.table(paste0(Prefix,"part5/",input4),sep="\t",header=TRUE) #TcgaTargetGTEX_phenotype.txt
...
withphenotypeclinical <- withphenotypeclinical %>% dplyr::select(sample,detailedcategory,sampletype, everything())
...
tcga2 <- tcga2 %>% dplyr::select(Type, everything())
...
withphenotype <- withphenotype %>% dplyr::select(sample,detailedcategory,sampletype, everything())
...
gtex2 <- gtex2 %>% dplyr::select(Type,sample,detailedcategory,sampletype, stage, everything()) #place the last column to first column


tcga_gtex<-rbind(tcga3,gtex2)
tcga_gtex2<-tcga_gtex[order(tcga_gtex$SETDB2),] #Sort by any gene to see if there is any data point that has off pattern
tcga_gtex2<- subset(tcga_gtex2, tcga_gtex2$SETDB2 > -9.9657) #Remove the rows that has -9.9658 values
...
library(ggplot2)
pdf(file=paste0(Prefix,"part5/stage/SETDB1.violin.stage.pdf"),height=50,width=7)
print(ggplot(data=final, aes(x=stage, y=SETDB1, fill=stage)) +
...
pdf(file=paste0(Prefix,"part5/stage/",paste(colnames(dat)[i],"violinstageplot.logtpm.pdf",sep=".")),height=35,width=5)
...
```{r}
#Do it for only SETDB1
henmt<- final2[,c("Type", "sampletype", "stage","SETDB1")]
library(data.table)
henmt2 <- data.table(henmt)
henmt2<- henmt2[, median:=median(SETDB1), by=list(Type, stage)]
...
pdf(file=paste0(Prefix,"part5/stage/SETDB1.line.stage.grouped.pdf"),height=5,width=5)
...
lage<- final2[,c("Type", "sampletype", "stage","KMT5B")]
library(data.table)
lage2 <- data.table(lage)
lage2<- lage2[, median:=median(KMT5B), by=list(Type, stage)]
...
pdf(file=paste0(Prefix,"part5/stage/KMT5B.line.stage.grouped.pdf"),height=5,width=5)
