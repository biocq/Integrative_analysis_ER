---
title: "TCGA-GTEX DATA MANIPULATION"
...
input1 <- "TcgaTargetGtex_rsem_gene_tpm_selected_genes.tsv" #"RMLP.TcgaTargetGtex_rsem_gene_tpm_withheader.tsv"
...
data<-read.delim(paste0(Prefix,"part5/",input1))#RMLP.TcgaTargetGtex_rsem_gene_tpm_withheader.tsv
...
phenotype<- read.table(paste0(Prefix,"part5/TcgaTargetGTEX_phenotype.txt"),sep="\t",header=TRUE) #read the phenotype table
...
write.table(withphenotype3,file=paste0(Prefix,"part5/ER.TcgaTargetGtex_tpm_with_phenotype.tsv"),row.names=FALSE, quote=FALSE, sep="\t",col.names=TRUE) #export
...
tcga_gtex <- tcga_gtex %>% dplyr::select(Type, everything()) #place the last column to first column
...
write.table(tpmfile,file=paste0(Prefix,"part5/TCGA_GTEX_FINAL.TPM.tsv"),quote=FALSE, sep="\t",row.names=FALSE)
#dat3 is log2(TPM+1)
dat3<- log2(dat2+1)
#Merge the processed log2(TPM+1) with the first initial two columns 
logfile <- cbind (dat[,1:4],dat3)
write.table(logfile,file=paste0(Prefix,"part5/TCGA_GTEX_FINAL.log2.tsv"),quote=FALSE, sep="\t",row.names=FALSE)


#IF you want to remove some cancer types from your file
remove <- c("DLBC", "CHOL", "THYM")
tpmfile2<- tpmfile[!tpmfile$Type %in% remove, ]
write.table(tpmfile2,file=paste0(Prefix,"part5/TCGA_GTEX_FINAL.TPM.without3cancer.tsv"),quote=FALSE, sep="\t",row.names=FALSE)
logfile2<- logfile[!logfile$Type %in% remove, ]
write.table(logfile2,file=paste0(Prefix,"part5/TCGA_GTEX_FINAL.log2.without3cancer.tsv"),quote=FALSE, sep="\t",row.names=FALSE)
